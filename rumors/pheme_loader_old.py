...
def _walk_structure(node: dict, parent_id: str | None = None) -> Iterator[tuple[str, str | None]]:
    """
    ...
    for tweet_id, children in node.items():
        ...
        if isinstance(children, dict):
            yield from _walk_structure(children, parent_id=tweet_id)
...
class PhemeLoader:
    ...
    def parse_thread(self, thread_dir: Path) -> dict | None:
        ...
        if structure_path.exists():
            ...
            for tid, pid in _walk_structure(structure):
    ...
    def load_all(self, verbose: bool = True) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        ...
        if verbose:
            print(f"Parsed {n_ok} threads, skipped {n_skip}.")
